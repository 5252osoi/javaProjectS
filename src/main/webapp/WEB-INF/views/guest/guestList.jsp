	...
	<div class="container">
			...
			<tr>
				<td>
					<c:if test="${sAdmin != 'adminOk'}">
						<a href="adminLogin" class="btn btn-secondary">관리자</a>
					</c:if>
					<c:if test="${sAdmin == 'adminOk'}">
						<a href="adminLogout" class="btn btn-secondary">관리자 로그아웃</a>
					</c:if>
				</td>
				<td class="text-right"><a href="guestInput" class="btn btn-info">글쓰기</a></td>
			</tr>
		</table>
		<table class="table table-borderless m-0 p-0">
			<tr>
				...
				<td class="text-right">
					...
					<c:if test="${pag>1}">
						<a href="guestList?pag=${pag-1}&pageSize=${pageSize}" title="이전퍼이지"><</a>
					</c:if>
					${pag}/${totPage}
		...
		<div class="text-center">
			<ul class="pagination justify-content-center" style="margin:20px 0">
				<c:if test="${pag>1}">
					<li class="page-item">
						<a class="page-link" href="guestList?pag=1&pageSize=${pageSize}" title="첫페이지">첫페이지</a>
					</li>
				</c:if>
				<li class="page-item">
				...
				<c:if test="${pag<totPage}">
					<li class="page-item">
						<a class="page-link" href="guestList?pag=${totPage}&pageSize=${pageSize}" title="마지막페이지">마지막페이지</a>
					</li>
				</c:if>
			</ul>
		</div>
