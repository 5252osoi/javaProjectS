...
<head>
	...
	<script>
		...
		function fNewForm(){
			...
			str+=	'</tr>';
			str+=	'<tr>';
			str+=		'<td colspan="2" class="text-center"><input type="submit" value="회원가입" class="btn btn-success mr-3">';
			str+=		'<input type="button" onclick="fNewFormHide()" value="돌아가기" class="btn btn-warning"></td>';
			str+=	'</tr>';
			str+='</table>';
			str+='</form>'
			document.getElementById("demo").innerHTML=str;
		}
		function fNewFormHide(){
	...
	<table class="table table-hover text-center">
		...
		<c:forEach var="vo" items="${vos}" varStatus="st">
			<tr>
				...
				<td>${vo.age}</td>
				<td>${vo.address}</td>
				<td><a href="#" onclick="userUpdate('${vo.idx}','${vo.name}','${vo.age}','${vo.address}')" data-toggle="modal" data-target="#myModal" class="btn btn-primary btn-sm">수정</a>
				<a href="javascript:userDelete(${vo.idx})" class="btn btn-danger btn-sm">삭제</a></td>
			</tr>
		</c:forEach>
